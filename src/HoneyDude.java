...
public class HoneyDude extends EntityMoving{
    ...
    public Point nextPosition(WorldModel world, Point destPos)
    {
        ...
        Predicate<Point> canPassThrough = p1 -> {return (world.withinBounds(p1) && !world.isOccupied(p1))
                || (world.withinBounds(p1) &&  world.getOccupancyCell(p1).getClass() == Beehive.class);};
            ...
            EventScheduler scheduler)
    {
        Optional<Entity> target =
                world.findNearest(this.getPosition(), new ArrayList<>(Arrays.asList(Beehive.class)));
        if (target.isPresent()) {
            this.moveTo(world, target.get(), scheduler);
            world.setBackground(getPosition(), new Background("honey_" + this.getId(), imageStore.getImageList("honey")));
        }
        scheduler.scheduleEvent(this,
